...
class PDFview(View):
    def get(self, request):
        ...
        # data = json.loads(self.request.body.decode('utf-8'))
        try:
            data = json.loads(request.META['HTTP_DATA'])
        except BaseException:
            return []
        #indexs keys, cats, years, domains
        print(data)
        ...
        print(current, res)
        if res is None:
            tmp = User.objects.filter(id__in = [i['_user__user__id'] for i in current.filter(_verified=True).only('_user__user__id').values('_user__user__id').distinct()])
        else:
            tmp = User.objects.filter(id__in = [i['_user__user__id'] for i in res.filter(_verified=True).only('_user__user__id').values('_user__user__id').distinct()])
